    ...
    try:
        ...
        if data.startswith("lang"):  # getLang
            ...
            if data == "thumb":
                ...
                if getThumb.photo:
                    try:
                        ...
                        logger.debug(f"Error saving thumbnail: {e}")
                        error_msg, _ = await util.translate(
                            text="SETTINGS['error']", lang_code=lang_code
                        )
                        await callbackQuery.answer(error_msg, show_alert=True)
                        await util.try_delete_message(getThumb)
                        return
                else:
                    no_photo, _ = await util.translate(
                        text="SETTINGS['error']", lang_code=lang_code
                    )
                    await callbackQuery.answer(no_photo, show_alert=True)
                await util.try_delete_message(getThumb)
                ...
                try:
                    ...
                    logger.debug(f"Error removing thumbnail: {e}")
            ...
            if data == "api":  # DATA saved order ["api", "fname", "caption"]
                ...
                await util.try_delete_message(getName)
            elif data == "api-":
                DATA[user_id][0] = 0
