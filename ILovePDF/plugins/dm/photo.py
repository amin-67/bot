...
async def _hd(bot, message):
    try:
        ...
        if message.chat.id in HD:
            # بدل الحذف، أزل الأزرار ووضح للمستخدم
            await util.disable_buttons_and_notify(message, "تم تعطيل الأزرار، لا يمكن تكرار الأمر")
            return
        ...
        # بدل الحذف، أزل الأزرار ووضح للمستخدم
        await util.disable_buttons_and_notify(message, "تم تعطيل الأزرار، انتهى التنفيذ")
        return
