...
async def disable_buttons_and_notify(message, text: str = None) -> None:
    """
    Safely disable inline buttons on a message and optionally replace the text
    with a short notice. This avoids deleting messages unexpectedly.

    - `message`: a pyrogram Message object
    - `text`: optional string to replace message text with (if None, will remove buttons only)
    """
    try:
        if text:
        ...
        try:
            ...
            # last-resort: try deleting silently (catch errors)
            try:
...
async def try_delete_message(message) -> bool:
    """
    Try to delete a message but never raise. Returns True if deletion succeeded.
    """
    try:
        await message.delete()
        return True
