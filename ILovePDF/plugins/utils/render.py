...
async def header(
    ...
    try:
        ...
        # فقط إذا كان الخطأ بسبب فقدان reply_to_message، أظهر التنبيه
        if hasattr(Error, 'args') and Error.args and 'reply_to_message' in str(Error.args[0]):
            try:
                await callbackQuery.edit_message_text(
                    text="⚠️ الرسالة الأصلية تم حذفها أو انتهت صلاحيتها. الأزرار تم تعطيلها.",
                    reply_markup=None,
                )
            except Exception:
                try:
                    await callbackQuery.answer("الرسالة الأصلية غير متوفرة.", show_alert=True)
                except Exception:
                    pass
            return "delete"
        # في باقي الأخطاء، فقط سجل الخطأ ولا تعطل الأزرار
        logger.exception(f"[header] Unexpected error: {Error}")
        return False
